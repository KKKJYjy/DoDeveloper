...
public class StudyServiceImpl implements StudyService {
	 ...
	 * @description : 1) 검색어가 있고 + 필터가 있는 경우 
	 * 2) 검색어가 있고 + 필터가 없는 경우 
	 * 3) 검색어가 없고 + 필터가 있는 경우 
	...
	public Map<String, Object> selectAllList(SearchStudyDTO sDTO, int pageNo) throws Exception {
		...
		}else if(sDTO.getSearchType().equals("") && sDTO.getSearchType().equals("") && sDTO.getStatusFilter() == null) {
		...
		}else if (sDTO.getSearchType().equals("") && sDTO.getSearchValue().equals("") && !(sDTO.getStatusFilter().equals(""))) {
	 ...
	 * @description : 유저가 필터링할 스터디 언어를 여러개 선택하면, 해당하는 스터디 모임글 리스트와 해당 모임글의 스터디 언어 리스트를 반환해준다.
	 */
	@Override
