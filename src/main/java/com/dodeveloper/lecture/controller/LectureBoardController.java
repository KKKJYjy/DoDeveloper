...
public class LectureBoardController {
     ...
     * @param : @RequestParam(value = "lecNo", defaultValue = "1") int lecNo int
     *          매개변수 'lecNo'가 존재하지만 기본 유형으로 선언되기 때문에 null값으로 변환할 수 없어서 객체 래퍼로 선언한 것
     * @param : LectureSearchDTO lsDTO - 검색어 Type와 Value를 가져옴
     * @return : void
     * @throws Exception
     * @description : 1) 강의 추천 게시판 전체 글 조회를 담당하는 controller 메서드 2) 검색 조건 : 검색을 했을 경우
     *              - 제목 / 작성자 / 내용
     */
...
//	    }
     ...
     * @author :
     * @date : 2024.05.18
     * @param : @RequestBody Map<String, String> likeRequest json 데이터를 자바의 Map 형태로
     *          매핑(lecNo가 int형이라 넘길수가 없기에 Map 형태로 String으로 만들어서 넘겼다.)
     * @return : ResponseEntity<String> - 문자열을 응답 본문으로 가지는 객체 / 그 중 Stirng 타입(문자열)으로
     *         응답
     ...
     * @return : ResponseEntity<String> - 문자열을 응답 본문으로 가지는 객체 / 그 중 Stirng 타입(문자열)으로
     *         응답
     * @description : 로그인 한 유저인 경우만 좋아요를 누를 수 있다. 유저가 하트를 한번 더 눌렀을 경우 1감소 -> ♡
     */
    @PostMapping("/unLike")
