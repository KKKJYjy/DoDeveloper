...
public class LectureBoardServiceImpl implements LectureBoardService {
     ...
     * @description : 1) 게시판 전체 조회에 대한 서비스 메서드 2) 검색 조건을 선택하고 검색어를 입력했을 때 글을 가져오는
     *              메서드 - 검색 조건 3) 검색 필터(최신순 / 인기순 / 조회순)을 선택했을 때 글을 가져오는 메서드 - 검색
     *              필터
     */
    @Override
	...
	// 검색 조건 처리
	// searchType이 null이 아니거나 / (!를 사용하여) searchValue가 비어있지 않은 경우를 isEmpty()를 사용해서
	// 확인
     ...
     * @return : void
     * @description : 페이징 처리하는 메서드 1) 검색 조건이 있을 경우 검색 조건에 대한 글의 갯수 가져오기 2) 검색필터의 조건이
     *              있는 경우 글의 갯수 가져오기
     */
     ...
     * @param : int pageNo - 보여줘야 할 페이지 번호
     * @return : void
     * @description : 검색어가 없는 경우의 페이징 처리 1) 페이징 구현할 때 필요한 값을 만들어 paginInfo 객체에 저장해야
     *              한다. 2) 페이징 블럭까지 처리해야한다.
     */
...
//		System.out.println("검색어가 없는 경우 : " + lDao.selectTotalLectureBoardCnt());
     ...
     * @description : 1) user가 조회하는 글을 하루 이내에 읽은 적이 있는지 없는지 검사 2) 하루 이내에 읽은 적이 없다 ->
     *              조회수 증가(update), 조회이력 - 기록을 남긴다(insert), 글 가져옴(유저가 글을 조회하도록) 3)
     *              하루 이내에 읽은 적이 있다 -> 게시글만 가져옴(유저가 글을 조회하도록)
     */
    @Override
     ...
     * @return : boolean
     * @description : 로그인 한 유저 + 좋아요를 눌렀던 유저일 경우만 좋아요를 취소할 수 있다. 유저가 하트를 한번 더 눌렀을 경우
     *              1감소 -> ♡
     */
    @Override
    @Transactional(propagation = Propagation.REQUIRED, isolation = Isolation.DEFAULT, rollbackFor = Exception.class)
